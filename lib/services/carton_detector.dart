// lib/services/carton_detector.dart
...
class CartonDetector {
  ...
  Future<void> loadModel() async {
    _interpreter ??= await tfl.Interpreter.fromAsset(
      'assets/models/carton.tflite',
    );
    final input = _interpreter!.getInputTensor(0);
    final output = _interpreter!.getOutputTensor(0);
    _inputShape = input.shape; // [1, 640, 640, 3]
    _outputShape = output.shape; // [1, 5, 8400]
  }

  Future<List<Detection>> detectCartons(img.Image decoded) async {
    if (!isLoaded) {
      await loadModel();
    }
    ...
    final resized =
        img.copyResize(decoded, width: modelW, height: modelH);
  ...
  List<Detection> _parseDetections(
    ...
    const double confThreshold = AppConfig.cartonConf;
    ...
    final scaleX = origW / modelW;
    final scaleY = origH / modelH;
